...
export function EditorCanvas(props: Props) {
  ...
  return <main className="flex h-screen flex-col overflow-hidden bg-[#eef1eb] text-[#17231c]">
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-[#d8e0d5] bg-[#f6f7f2] px-5">
      ...
      <div className="flex min-w-0 items-center gap-3"><span role="status" aria-live="polite" title={props.error || props.notice || undefined} className={`max-w-[120px] truncate text-xs sm:max-w-[320px] ${props.error ? 'font-semibold text-[#c94d32]' : 'text-[#627067]'}`}>{props.error || props.notice || (props.saveState === 'saving' ? 'Saving changes...' : 'All changes saved')}</span><button type="button" onClick={props.onPreview} className="inline-flex items-center gap-2 rounded-lg border border-[#ccd8ca] bg-white px-3 py-2 text-xs font-bold hover:border-[#17231c] hover:bg-[#f6f7f2]"><EyeIcon className="h-4 w-4" /> Preview</button><button type="button" onClick={props.onPublish} disabled={props.isPublishing} aria-busy={props.isPublishing} className="inline-flex items-center gap-2 rounded-lg bg-[#e25d3f] px-4 py-2 text-xs font-bold text-white hover:bg-[#c94d32] disabled:cursor-not-allowed disabled:opacity-50">{props.isPublishing ? <span className="spinner" aria-hidden /> : <RocketLaunchIcon className="h-4 w-4" />} {props.isPublishing ? 'Publishing...' : 'Publish'}</button></div>
    </header>
