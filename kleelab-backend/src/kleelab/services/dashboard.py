...
async def get_user_stats(user_id: UUID, db: AsyncSession) -> dict:
    ...
    views = await db.scalar(select(func.count()).select_from(AnalyticsEvent).where(AnalyticsEvent.site_id.in_(site_ids), AnalyticsEvent.created_at >= datetime.now(timezone.utc) - timedelta(days=30)))
